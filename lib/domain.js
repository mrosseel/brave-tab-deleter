// Common second-level domains that indicate a two-part TLD when followed by a country code
const COMMON_SLDS = ['com', 'co', 'org', 'net', 'gov', 'edu', 'ac', 'or', 'ne', 'go'];

/**
 * Check if domain ending looks like a two-part TLD (e.g., com.be, co.uk)
 * Pattern: common SLD + 2-letter country code
 */
export function isTwoPartTLD(lastTwo) {
  const parts = lastTwo.split('.');
  if (parts.length !== 2) return false;
  const [sld, tld] = parts;
  // Check if SLD is common and TLD is a 2-letter country code
  return COMMON_SLDS.includes(sld) && tld.length === 2;
}
...
export function getShortName(domain) {
  ...
  if (parts.length >= 2) {
    // Handle two-part TLDs like co.uk, com.be
    const lastTwo = parts.slice(-2).join('.');
    if (isTwoPartTLD(lastTwo) && parts.length >= 3) {
...
export function getDomain(url) {
  try {
    ...
    // Handle common two-part TLDs (co.uk, com.au, com.be, etc.)
    const lastTwo = parts.slice(-2).join('.');

    if (isTwoPartTLD(lastTwo)) {
